...
from core.model.fusion_classifier import MobileNetV3PoseFusion
from core.utils.fusion_dataset import TrafficGestureFusionDataset
from core.utils.pose_features import estimate_pose_feature_dim
...
def parse_args():
    parser = argparse.ArgumentParser(description="Train a MobileNetV3 + Pose fusion classifier.")
    parser.add_argument(
        "--config",
        default=str(PROJECT_ROOT / "config" / "fusion_mobilenetv3_pose.yaml"),
...
def build_albumentations_transforms(config: dict):
    ...
    train_tf = A.Compose(
        [
            ...
            A.OneOf(
                [
                    A.RandomRain(slant_range=(-10, 10), drop_length=16, drop_width=1, blur_value=3, brightness_coefficient=0.9, p=1.0),
...
def create_dataloaders(config: dict):
    ...
    train_dataset = TrafficGestureFusionDataset(
        dataset_root=dataset_root,
        split="train",
        pose_json_path=pose_root / "train_pose_keypoints.json",
        transform=train_tf,
    )
    valid_dataset = TrafficGestureFusionDataset(
        dataset_root=dataset_root,
        split="valid",
        pose_json_path=pose_root / "valid_pose_keypoints.json",
        transform=eval_tf,
    )
    test_dataset = TrafficGestureFusionDataset(
        dataset_root=dataset_root,
        split="test",
        pose_json_path=pose_root / "test_pose_keypoints.json",
        transform=eval_tf,
    )
...
def train_one_epoch(model, loader, criterion, optimizer, device):
    ...
    for images, pose_features, targets in loader:
...
def build_run_dir(config: dict):
    ...
    run_name = config["train"].get("run_name") or datetime.now().strftime("fusion_%Y%m%d_%H%M%S")
...
def write_training_log(rows: list[dict], path: Path):
    fieldnames = ["epoch", "train_loss", "train_acc", "valid_loss", "valid_acc", "lr"]
...
def main():
    ...
    pose_dim = estimate_pose_feature_dim()

    model = MobileNetV3PoseFusion(
        num_classes=len(class_names),
        pose_input_dim=pose_dim,
        pretrained=config["model"]["pretrained"],
        image_dropout=config["model"]["dropout"],
        pose_hidden_dim=config["model"]["pose_hidden_dim"],
        fusion_hidden_dim=config["model"]["fusion_hidden_dim"],
        pose_dropout=config["model"]["pose_dropout"],
    ).to(device)
    ...
    save_json({"class_names": class_names, "pose_feature_dim": pose_dim}, run_dir / "class_names.json")
