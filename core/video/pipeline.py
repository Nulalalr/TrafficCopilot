...
class VideoPipeline:
    ...
    def run(self) -> dict[str, Any]:
        ...
        intent = self.system.new_intent_engine()
        intent.reset()
        self.tracker.reset()
            ...
            while True:
                ...
                if tracks:
                    ...
                    if roi.size > 0:
                        ...
                        pred = self.system.predictor.predict(pil)
                        event["prediction"] = pred.as_dict()
                        event["intent"] = intent.update(pred.as_dict())

                        if draw:
                            self._draw_track(frame, track_id=int(track.track_id), bbox=(x1, y1, x2, y2))
                            self._draw_text(frame, pred.label, pred.confidence, event["intent"])
